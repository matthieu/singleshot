...
h2(#lists). Mailing Lists

Come join us on the "Singleshot Google Group":http://groups.google.com/group/singleshot.
...
h2(#migration). Database Migration
...
<pre>$ rake db:migrate:reset db:test:clone</pre>

This task will recreate the entire development and test database.  In addition, we use "Annotates Models":http://github.com/ctran/annotate_models/tree/master to, well, annotate the Rails models with the most recent database schema.  So when changing the database schema, follow
with:
